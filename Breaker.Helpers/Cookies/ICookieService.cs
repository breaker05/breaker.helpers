	...
	public class CookieService : ICookieService
	{
			...
			where T : class
		{
			return ExceptionHandler.SwallowOnException(() =>
			{
				...
				if (_pendingCookies.TryGetValue(cookieName, out CachedCookie cookie))
				{
					...
					return isBase64 ? JsonSerializer.Deserialize<T>(cookie.Value.FromBase64String())
						: JsonSerializer.Deserialize<T>(cookie.Value);
				}

				if (_httpContext.Request.Cookies.TryGetValue(cookieName, out string cookieValue))
					return isBase64 ? JsonSerializer.Deserialize<T>(cookieValue.FromBase64String())
			...
			where T : class
		{
			...
			cookie.Value = base64Encode
						? JsonSerializer.Serialize(data).ToBase64String()
